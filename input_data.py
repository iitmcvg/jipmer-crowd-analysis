...
def _corrupt_brightness(image, mask, count):
...
def _corrupt_contrast(image, mask, count):
...
def _corrupt_saturation(image, mask, count):
...
def _flip_left_right(image, mask, count):
